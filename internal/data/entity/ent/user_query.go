...
func (_q *UserQuery) QueryUserPositions() *UserPositionQuery {
...
func (_q *UserQuery) QueryUserDepartments() *UserDepartmentQuery {
	query := (&UserDepartmentClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserDepartments: _q.withUserDepartments.Clone(),
...
func (_q *UserQuery) WithUserPositions(opts ...func(*UserPositionQuery)) *UserQuery {
	query := (&UserPositionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithUserDepartments(opts ...func(*UserDepartmentQuery)) *UserQuery {
	query := (&UserDepartmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserDepartments = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withUserPositions; query != nil {
		if err := _q.loadUserPositions(ctx, query, nodes,
			func(n *User) { n.Edges.UserPositions = []*UserPosition{} },
			func(n *User, e *UserPosition) { n.Edges.UserPositions = append(n.Edges.UserPositions, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUserDepartments; query != nil {
		if err := _q.loadUserDepartments(ctx, query, nodes,
...
func (_q *UserQuery) loadUserPositions(ctx context.Context, query *UserPositionQuery, nodes []*User, init func(*User), assign func(*User, *UserPosition)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userposition.FieldUserID)
	}
	query.Where(predicate.UserPosition(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserPositionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadUserDepartments(ctx context.Context, query *UserDepartmentQuery, nodes []*User, init func(*User), assign func(*User, *UserDepartment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
