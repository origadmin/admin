...
func (_q *DepartmentQuery) QueryPositions() *PositionQuery {
...
func (_q *DepartmentQuery) QueryUserDepartments() *UserDepartmentQuery {
...
func (_q *DepartmentQuery) WithPositions(opts ...func(*PositionQuery)) *DepartmentQuery {
	query := (&PositionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) WithUserDepartments(opts ...func(*UserDepartmentQuery)) *DepartmentQuery {
	query := (&UserDepartmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) loadPositions(ctx context.Context, query *PositionQuery, nodes []*Department, init func(*Department), assign func(*Department, *Position)) error {
...
func (_q *DepartmentQuery) loadUserDepartments(ctx context.Context, query *UserDepartmentQuery, nodes []*Department, init func(*Department), assign func(*Department, *UserDepartment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
