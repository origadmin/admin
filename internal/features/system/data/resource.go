...
func (repo *resourceRepo) Get(ctx context.Context, id int64, options ...dto.ResourceQueryOption) (*types.Resource, error) {
	result, err := repo.db.Resource.Get(ctx, int(id))
	if err != nil {
		return nil, err
	}
...
func (repo *resourceRepo) Create(ctx context.Context, r *types.Resource, options ...dto.ResourceQueryOption) (*types.Resource, error) {
	if r.ParentId > 0 {
		parent, err := repo.db.Resource.Get(ctx, int(r.ParentId))
		if err != nil {
			return nil, err
		}
		r.TreePath = parent.TreePath + strconv.Itoa(parent.ID) + repo.Delimiter
	}

	create := repo.db.Resource.Create().
		SetName(r.Name).
		SetParentID(int(r.ParentId)).
