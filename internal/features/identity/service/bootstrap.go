...
// PolicyBootstrap is responsible for bootstrapping identityorization policies at service startup.
...
func NewPolicyBootstrap(syncer *biz.PolicySyncer, logger log.Logger) *PolicyBootstrap {
	return &PolicyBootstrap{
		syncer: syncer,
		log:    log.NewHelper(log.With(logger, "module", "identity.service.policy_bootstrap")),
	}
}

// Bootstrap performs one-time initialization of identityorization policies with a retry mechanism.
...
func (b *PolicyBootstrap) Bootstrap(ctx context.Context) error {
	b.log.WithContext(ctx).Info("Starting identityorization policy bootstrap...")
